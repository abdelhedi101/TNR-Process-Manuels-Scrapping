    ...
    def _fill_once() -> bool:
        ...
        ok_local &= fill_consultation_field(
            ...
            [
                ...
                "//label[normalize-space()='Position Basis']/ancestor::tr//input[contains(@class,'x-form-field')]",
            ],
            criteria.get("position_basis", "businessdate"),
            "Position Basis",
            settle_ms=60,
        )
        ok_local &= fill_consultation_field(
...
def execute_megacommon_consultation(page, criteria: Dict[str, str]) -> bool:
    ...
    try:
        execute_button.click(force=True, timeout=4000)
        page.wait_for_timeout(400)
...
def find_tree_node(
    ...
    *,
    ...
    exact_pattern = re.compile(rf"^\s*{re.escape(target_label)}\s*$", re.IGNORECASE)
    try:
        exact_node = treeitems.filter(has_text=exact_pattern).first
        if exact_node.count():
            try:
                exact_node.scroll_into_view_if_needed()
            except PlaywrightError:
                pass
            return exact_node
    except PlaywrightError:
...
def find_login_or_app_page(page, app_ready_selector: str, login_selectors=None):
    if login_selectors is None:
        login_selectors = ["#username", "input[name='username']", "input[name='j_username']"]

    try:
        context = page.context
    except Exception:
        return get_alive_page(page)
    ...
    for candidate in reversed(list(context.pages)):
        try:
            ...
            if candidate.locator(app_ready_selector).count():
                logging.info("Selected app-ready page after login from context: %s", candidate.url)
                return candidate

            for selector in login_selectors:
                if candidate.locator(selector).count():
                    logging.info("Selected login page after login from context: %s", candidate.url)
                    return candidate
...
def login(page, entry_url: Optional[str] = None) -> bool:
    ...
    page = find_login_or_app_page(page, app_ready)
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if common_page and not common_page.is_closed():
                ...
                # Toujours refaire la saisie des champs après relogin
                if relogged:
                    if not prepare_megacommon_consultation(common_page, pre_criteria):
                        logging.error("MegaCommon field fill failed after relogin; cannot execute final search")
                        return
