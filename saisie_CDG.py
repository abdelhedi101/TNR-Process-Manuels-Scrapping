...
def capture_saisie_save_popup_screenshot(page, module_name: str, path_label: str) -> None:
    target_dir = SCREENSHOT_DIR / "CDG" / "saisie_instruction_unitaire"
...
def fill_saisie_instruction_client_form(page, module_name: str, path_label: str) -> bool:
    ...
    def _set_field_value(field, value_to_set: str) -> bool:
        try:
            field.evaluate(
            ...
            pass
        try:
            field.fill(value_to_set)
            return True
        except Exception:
            pass
        return False

    def _commit_field(field) -> None:
        try:
            field.evaluate(
    ...
    def _select_combo_option(trigger_locator, value_to_set: str) -> bool:
        try:
            trigger_locator.click(force=True, timeout=4000)
            page.wait_for_timeout(300)
        except Exception:
            return False
        text_pattern = re.compile(rf"^\s*{re.escape(value_to_set.strip())}\s*$", re.IGNORECASE)
        for sel in ["div[role='listitem'].x-combo-list-item", "div.x-boundlist-item", "li.x-boundlist-item"]:
            option = page.locator(sel).filter(has_text=text_pattern).first
            if option.count():
                try:
                    option.click(force=True, timeout=4000)
                    page.wait_for_timeout(400)
    ...
    def _restore_previous_fields(previous_fields: List[dict]) -> None:
        for prev in previous_fields:
            prev_field = prev.get("field")
            expected = prev.get("expected", "")
            label = prev.get("label", "")
            if not prev_field or not prev_field.count() or not expected:
                continue
            actual = _get_field_value(prev_field)
            if actual != expected:
                logging.warning("Field '%s' was cleared; restoring value '%s'", label, expected)
                _set_field_value(prev_field, expected)
                page.wait_for_timeout(300)
                _commit_field(prev_field)
                page.wait_for_timeout(400)

    def _fill_one_field(spec: dict, value: str, next_selector: Optional[str], previous_fields: List[dict]) -> bool:
        key = spec["key"]
        label = spec["label"]
        ...
        try:
            ...
            logging.warning("Click on field '%s' timed out for %s", label, path_label)
            return False

        if key == "otc_traded":
            trigger = page.locator("#Component_PAGE_FORM_0_oTCTraded img.x-form-trigger-arrow").first
            if not trigger.count():
                trigger = field.locator(
                    "xpath=ancestor::div[contains(@class,'x-form-field-wrap')][1]//img[contains(@class,'x-form-trigger-arrow')]"
                ).first
            if trigger.count() and _select_combo_option(trigger, value):
                logging.info("Selected OTC Traded = %s via dropdown", value)
            else:
                _set_field_value(field, value)
                logging.info("Filled OTC Traded = %s (fallback)", value)

        elif key == "transaction_type":
            trigger = page.locator("#Component_PAGE_FORM_0_transactionType img.x-form-trigger-arrow").first
            if not trigger.count():
                trigger = field.locator(
                    "xpath=ancestor::div[contains(@class,'x-form-field-wrap')][1]//img[contains(@class,'x-form-trigger-arrow')]"
                ).first
            if trigger.count() and _select_combo_option(trigger, value):
                logging.info("Selected Transaction Type = %s via dropdown", value)
            else:
                _set_field_value(field, value)
                logging.info("Filled Transaction Type = %s (fallback)", value)
        ...
        # Validate and retry once for plain text fields
        if key not in ("otc_traded", "transaction_type"):
            current = _get_field_value(field)
            if current != value:
                logging.info("Retry field '%s': expected '%s', got '%s'", label, value, current)
                try:
                    field.click(force=True, timeout=4000)
                    field.fill(value)
        ...
        _restore_previous_fields(previous_fields)

        logging.info("Filled field '%s' = '%s'", label, value)

        # Navigate directly to next field by click; fallback to Tab
        if next_selector:
            navigated = False
            for sel in _split_selectors(next_selector):
                try:
    ...
    fillable = [spec for spec in SAISIE_FIELD_SPECS if variables.get(spec["key"], "").strip()]
    filled_so_far: List[dict] = []

    for idx, spec in enumerate(fillable):
        value = variables[spec["key"]].strip()
        next_selector = fillable[idx + 1]["selector"] if idx + 1 < len(fillable) else None

        if not _fill_one_field(spec, value, next_selector, filled_so_far):
            capture_failure(page, module_name, path_label, always=True)
            return False

        resolved_field = _find_field(spec["label"], spec["selector"])
    ...
    logging.info("All saisie fields filled for %s", path_label)
    ...
    try:
        ...
        save_button = next((c for c in save_button_candidates if c.count()), None)
    ...
    for idx in range(treeitems.count()):
        ...
        try:
            if not candidate.is_visible():
                continue
            candidate_text = (candidate.inner_text() or "").strip().lower()
            if candidate_text == target_label or target_label in candidate_text:
                return candidate
        except Exception:
            continue

    return treeitems.filter(has_text=re.compile(rf"^\s*{re.escape(label)}\s*$", re.IGNORECASE)).first
...
def traverse_menu_paths(page, menu_paths: List[List[str]]) -> None:
    ...
    for index, path in enumerate(menu_paths):
        ...
        if normalized_path == MARCHE_TRANSFER_PATH or normalized_path == MARCHE_CONSULTATION_PATH:
            ...
            handle_error_dialog(page, top_level, path_label)
        elif is_saisie_instruction_client_path(normalized_path):
            pass
        ...
        for child_index, segment in enumerate(path[1:], start=1):
            ...
            if node_is_leaf:
                ...
                if is_saisie_instruction_client_path(normalized_path):
