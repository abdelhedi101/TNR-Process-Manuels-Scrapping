...
CREATION_ROLE_ENTITE_VARIABLES_FILE = Path(
    os.getenv("CREATION_ROLE_ENTITE_VARIABLES_FILE", "variable_saisies/Creation_role_entite.txt")
)
...
def fill_affecter_role_entite_form(page, module_name: str, path_label: str) -> bool:
    entity_value = load_role_entite_value(ROLE_ENTITE_VARIABLES_FILE)
    if not entity_value:
        logging.warning("Role assignment entity value is empty in %s", ROLE_ENTITE_VARIABLES_FILE)
        capture_failure(page, module_name, path_label, always=True)
        return False
