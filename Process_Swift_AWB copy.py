...
def _right_click_first_row_voir_screenshot(page, screenshot_path: Path, return_after: bool = True) -> bool:
    ...
    try:
        ...
        log.warning("Right-click on row failed: %s", exc)
        return False
    ...
    for selector in [
        ...
        if candidate.count():
            try:
                ...
                continue
    if voir is None:
        log.warning("'Voir' menu item not visible")
        try:
            page.keyboard.press("Escape")
        except Exception:
            pass
...
def _screenshot_and_dismiss_popup(page, screenshot_path: Path) -> bool:
    ...
    # Priority 1: if it's an Error popup (header text == 'Error'), click the X
    # close icon (x-tool-close) on its header to dismiss.
    try:
...
def _validation_instruction_client(page, client_ref: str) -> bool:
    ...
    return _screenshot_and_dismiss_popup(page, screenshot_path)
